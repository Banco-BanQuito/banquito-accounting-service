...
public class RequestLoggingFilter extends OncePerRequestFilter {
    ...
    private void logCompletion(HttpServletRequest request, HttpServletResponse response, long durationMs) {
        ...
        if (durationMs >= 1_000) {
            log.warn("HTTP request exceeded observation threshold: method={}, path={}, durationMs={}", request.getMethod(), request.getRequestURI(), durationMs);
        }
        if (status >= 500) {
            log.error("HTTP request completed with server error: method={}, path={}, status={}, durationMs={}", request.getMethod(), request.getRequestURI(), status, durationMs);
        } else if (status >= 400) {
            log.warn("HTTP request completed with client error: method={}, path={}, status={}, durationMs={}", request.getMethod(), request.getRequestURI(), status, durationMs);
        } else if (status >= 300) {
            log.info("HTTP redirection observed: method={}, path={}, status={}, durationMs={}", request.getMethod(), request.getRequestURI(), status, durationMs);
        } else if (status >= 200) {
            log.info("HTTP request completed successfully: method={}, path={}, status={}, durationMs={}", request.getMethod(), request.getRequestURI(), status, durationMs);
        } else {
            log.warn("HTTP response status below success range: method={}, path={}, status={}, durationMs={}", request.getMethod(), request.getRequestURI(), status, durationMs);
        }
